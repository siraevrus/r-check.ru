...
$stmt = $pdo->prepare("
    ...
    WHERE DATE(s.sale_date) BETWEEN uh.period_from AND uh.period_to
    ORDER BY uh.period_from DESC
");
$stmt->execute([$doctorId]);
...
$stmt = $pdo->prepare("
    ...
    LEFT JOIN upload_history uh ON s.upload_period_id = uh.id
    WHERE u.id = ?
    GROUP BY uh.id, uh.period_from, uh.period_to, uh.upload_date, s.product_name, uh.description
    ORDER BY uh.upload_date DESC, s.product_name ASC
");
$stmt->execute([$doctorId]);
